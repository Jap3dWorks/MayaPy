# coding=utf-8
# coding=utf-8
# documentation: https://github.com/mmerchante/instanceAlongCurve/blob/master/instanceAlongCurve.py#L1156
# documentation: http://www.chadvernon.com/blog/resources/maya-api-programming/mrampattribute/
# for study purposes:
...
class instanceAlongCurveLocator(OpenMayaMPx.MPxLocatorNode):
    ...
    # Custom Manipulator container MPXManipContainer
    ...
    # class to create ramp attributes
    ...
    class RampValueContainer(object):
        ...
        def __init__(self, mObject, dataBlock, rampAttr, normalize, instanceCount):
            # Explanation: get function to operate with ramp attributes, isn't a dataBlock
            ...
            if amplitudePlug.isConnected():
                ...
                if connections.length()==1:
                    ...
                    if(node.hasFn(OpenMaya.MFn.kTexture2d)):
                        # Explanation: get samples from a texture
                        OpenMayaRender.MRenderUtilities.sampleShadingNetwork(nodeFn.name() + '.outColor', instanceCount,
                                                                             False, False, OpenMaya.MFloatMatrix(), None,
                                                                             uValues, vValues, None, None, None, None, None,
    ...
    def __init__(self):
        super(instanceAlongCurveLocator, self).__init__()

    def postConstructor(self):
        """
        When a attribute changes, calls self.attrChangeCallback and self.updateInstanceConnections
        """
    ...
    # TODO: maybe it can be precalculated?  <-- Try understand this
    def getAvailableLogicalIndices(self, plug, numIndices):
        ...
        for i in indices:
            ...
            # review: maybe I can do this with sets subtract without iterations
    ...
    def updateInstanceConnections(self):
        ...
        # actual elements connected
        numConnectedElements = outputTranslationPlug.numConnectedElements()

        # Only instance if we are missing elements
        # TODO: handle mismatches in translation/rotation plug connected elements (connectionBroken method?)
        if numConnectedElements < expectedInstanceCount:
            inputTransformFn = self.getInputTransformFn()  # OpenMaya.MFnTransform()
            ...
            if inputTransformFn is not None:
                ...
                for i in availableIndices:
                    ...
                    # connecting attributes of cloned objects to the node
    ...
    def getInstanceCountByMode(self):
        ...
        if inputCurvePlug.isConnected() and instancingModePlug.asInt() == 1:
            ...
            maxInstancesByLengthPlug = OpenMaya.MPlug(self.thisMObject(), instanceAlongCurveLocator.instanceLengthAttr)
    ...
    def getRandomizedValue(self, random, randomAmplitude, value):
        return (random.random() * 2.0 - 1) * randomAmplitude + value

    # calculate expected instances be the instancing mode
    ...
    def updateInstancePosition(self, curveFn, dataBlock, count, distOffset, curveStart, curveEnd, effectiveCurveLength, lengthIncrement, inputTransformPlug, inputTransformFn, axisHandlesSorted):
        ...
        curveForm = curveFn.form()  # kOpen, KClosed, KPeriodic or KInvalid

        # important: enums are short! if not, the resulting int may be incorrect
        rotMode = dataBlock.inputValue(instanceAlongCurveLocator.orientationModeAttr).asShort()  # int short
        localRotationAxisMode = dataBlock.inputValue(instanceAlongCurveLocator.inputLocalOrientationAxisAttr).asShort()  # int short
        ...
        if inputTransformPlug.isConnected():
            # review: rotatepivot and rotatepivotTranslation
        ...
        for i in xrange(min(count, translateArrayHandle.elementCount())):
            ...
            if referenceAxis.isParallel(tangent) and (referenceAxis * tangent < 0):
                ...
                # review: get rotation in quaternions
    ...
    def updateinstanceScale(self, curvefn, dataBlock, count, distOffset, curveStart, curveEnd, effectiveCurveLength, lengthIncrement):
        ...
        localScaleOffset = dataBlock.inputValue(instanceAlongCurveLocator.inputGlobalTranslationOffsetAttr.compound).asVector()
        ...
        # Make sure there are enough handless...
    ...
    def updateInstanceRotations(self, curvefn, dataBlock, count, distOffset, curveStart, curveEnd, effectiveCurveLength, lengthIncrement, inputTransformPlug, inputTransformFn, axisHandlesSorted):
        ...
        rotationArrayHandle = dataBlock.outputArrayValue(instanceAlongCurveLocator.ouputRotationAttr.compound)
        ...
        # Documentation: enums are short! If not, the resulting int may be incorrect
        ...
        for i in xrange(min(count, rotationArrayHandle.elementCount())):
            ...
            # If the axis is parallel, but with inverse direction, rotate it PI over the up vector
            if referenceAxis.isParallel(tangent) and (referenceAxis * tangent < 0):
                rot = OpenMaya.MQuaternion(math.pi, referenceUp)  # rotate pi referenceUp axis

            # Rotate local axis to align with tangent
            rot = localRotation * rot  # review: multiply quaternions, review MQuaternion?
            ...
            if rotMode == 0:    # identity
                ...
                rot = inputTransformRotation
            elif rotMode == 3 and i % 2 == 1:   #chainMode
                rot *= OpenMaya.MQuaternion(math.pi * .5, tangent)
        ...
        try:
            ...
            if not curve.isNull():
                if updateTranslation or updateRotation or updateScale:
                    ...
                    axisHandleSorted = getSortedCurveAxisArray(self.thisMObject(), curveAxisHandleArray, instanceCount)

                    if updateTranslation:
                        self.updateInstancePosition(curveFn, dataBlock, instanceCount, distOffset, curveStart, curveEnd,
                                                    effectiveCurveLength, lengthIncrement, inputTransformPlug, inputTransformFn,
                                                    axisHandleSorted)
                    if updateRotation:
                        self.updateInstanceRotations(curveFn, dataBlock, instanceCount, distOffset, curveStart, curveEnd,
                                                    effectiveCurveLength, lengthIncrement, inputTransformPlug, inputTransformFn,
                                                    axisHandleSorted)
                    if updateScale:
                        self.updateinstanceScale(curveFn, dataBlock, instanceCount, distOffset, curveStart, curveEnd,
                                                     effectiveCurveLength, lengthIncrement)
    ...
    def addCompoundvector3Attribute(cls, compoundAttribute, attributeName, unitType, arrayAttr, inputAttr, defaultValue):
        # Schematic View compoundAttribute:
        # compoundAttribute[?]
        #   compoundAttributeX
        #   compoundAttributey
        #   compoundAttributez
        ...
        # Output compound
        compoundAttribute.compound = nAttr.create(attributeName, attributeName, compoundAttribute.x, compoundAttribute.y, compoundAttribute.z)

        nAttr.setWritable(inputAttr)
        # Explanation: set compound Attr like an array
        # we can add or remove any number of connections
        nAttr.setArray(arrayAttr)
        nAttr.setUsesArrayDataBuilder(arrayAttr)  # review: setUsesArrayDataBuilder
        nAttr.setDisconnectBehavior(OpenMaya.MFnAttribute.kDelete)  # nothing connected, remove attr
    ...
    @staticmethod
    def nodeInitializer():
        #Explanation: Associate the node with its im manipulator
        ...
        node.inputTransformAttr = matrixAttrFn.create("inputTransformMatrix", "inputTransformMatrix",
                                                      OpenMaya.MFnMatrixAttribute.kFloat)
        ...
        # create attributes with a func @classMethod
        node.addCompoundVector3Attribute(node.inputLocalRotationOffsetAttr, "inputLocalRotationOffset",
                                         OpenMaya.MFnUnitAttribute.kDistance, False, True,
                                         OpenMaya.MVector(0.0, 0.0, 0.0))
        node.addCompoundVector3Attribute(node.inputGlobalRotationOffsetAttr, "inputGlobalRotationOffset",
                                         OpenMaya.MFnUnitAttribute.kDistance, False, True,
                                         OpenMaya.MVector(0.0, 0.0, 0.0))

        node.addCompoundVector3Attribute(node.inputGlobalTranslationOffsetAttr, "inputGlobalTranslationOffset",
                                         OpenMaya.MFnUnitAttribute.kDistance, False, True,
                                         OpenMaya.MVector(0.0, 0.0, 0.0))
        node.addCompoundVector3Attribute(node.inputLocalTranslationOffsetAttr, "inputLocalTranslationOffset",
                                         OpenMaya.MFnUnitAttribute.kDistance, False, True,
                                         OpenMaya.MVector(0.0, 0.0, 0.0))

        node.addCompoundVector3Attribute(node.inputLocalScaleOffsetAttr, "inputLocalScaleOffset",
                                         OpenMaya.MFnUnitAttribute.kDistance, False, True,
                                         OpenMaya.MVector(1.0, 1.0, 1.0))
        ...
        # add attributes with a @classMethod
        # Default translation ramp axis is UP
        node.addRampAttributes(node.positionRampAttr, "position", OpenMaya.MFnUnitAttribute.kDistance,
                               OpenMaya.MVector(0.0, 1.0, 0.0))

        # Default rotation ramp axis is TANGENT
        node.addRampAttributes(node.rotationRampAttr, "rotation", OpenMaya.MFnUnitAttribute.kDistance,
                               OpenMaya.MVector(0.0, 0.0, 1.0))

        # Default scale axis is uniform
        node.addRampAttributes(node.scaleRampAttr, "scale", OpenMaya.MFnUnitAttribute.kDistance,
                               OpenMaya.MVector(1.0, 1.0, 1.0))

        # Output attributes
        node.addCompoundVector3Attribute(node.outputTranslationAttr, "outputTranslation",
                                         OpenMaya.MFnUnitAttribute.kDistance, True, False,
                                         OpenMaya.MVector(0.0, 0.0, 0.0))
        node.addCompoundVector3Attribute(node.outputRotationAttr, "outputRotation", OpenMaya.MFnUnitAttribute.kAngle,
                                         True, False, OpenMaya.MVector(0.0, 0.0, 0.0))
        node.addCompoundVector3Attribute(node.outputScaleAttr, "outputScale", OpenMaya.MFnUnitAttribute.kDistance, True,
                                         False, OpenMaya.MVector(1.0, 1.0, 1.0))

        ## Input instance count
        node.enableManipulatorsAttr = nAttr.create("enableManipulators", "enableManipulators",
        ...
        ## Input handle count
        node.curveAxisHandleCountAttr = nAttr.create("curveAxisHandleCount", "curveAxisHandleCount",
...
# AE TEMPLATE #
###############
# with the AE template, we can add additional signals to the menu widgets.
# documentation: AE Template
...
class AEinstanceAlongCurveLocatorTemplate(pm.ui.AETemplate):
    # documentation: http://download.autodesk.com/us/maya/2009help/CommandsPython/editorTemplate.html#flagaddControl
    ...
    def __init__(self, nodeName):
        super(AEinstanceAlongCurveLocatorTemplate, self).__init__(nodeName)
        self.thisNode = None
        ...
        if self.node.type() == kPluginNodeName:
            ...
            annotation = 'If the locator mode is on Distance, this length will define the spacing between each instance. <br> <br> Note that if te curve length is greater than an integrer amount of distances, some space will be left unoccupied.'
            ...
            self.addControl('maxInstanceByLength', label='Max Instances', changeCommand=self.onInstanceModeChange, annotation=annotation)

            self.addSeparator()

            annotation = "An offset for the evaluation of the curve position/rotation. This also modifies the ramp evaluation. "
            self.addControl("distOffset", label="Curve Offset",
                            changeCommand=lambda nodeName: self.updateDimming(nodeName, "distOffset"),
            ...
            self.addControl("curveStart", label="Curve Start",
                            changeCommand=lambda nodeName: self.updateDimming(nodeName, "curveStart"),
                            annotation=annotation)
            ...
            self.addControl("curveEnd", label="Curve End",
                            changeCommand=lambda nodeName: self.updateDimming(nodeName, "curveEnd"),
                            annotation=annotation)
            ...
            self.addControl("orientationMode", label="Orientation Mode",
                            changeCommand=lambda nodeName: self.updateOrientationChange(nodeName),
                            annotation=annotation)

            annotation = "Each instance will be rotated so that this axis is parallel to the curve tangent."
            self.addControl("inputLocalOrientationAxis", label="Local Axis",
                            changeCommand=lambda nodeName: self.updateDimming(nodeName, "inputLocalOrientationAxis"),
                            annotation=annotation)
            ...
            self.addControl("enableManipulators", label="Enable manipulators",
                            changeCommand=lambda: self.updateManipCountDimming(nodeName),
                            annotation=annotation)
            # review: why nodeName: it's an argument yet, maybe is better a partial here
            ...
            self.addControl("curveAxisHandleCount", label="Manipulator count",
                            changeCommand=lambda nodeName: self.updateManipCountDimming(nodeName),
                            annotation=annotation)
            self.callCustom(lambda: self.buttonNew(nodeName), self.buttonUpdate, "curveAxisHandleCount")
            # review: why attr: and then nodeName?, maybe is better a partial here
            self.addSeparator()
            ...
            self.addControl("instanceDisplayType", label="Instance Display Type",
                            changeCommand=lambda nodeName: self.updateDimming(nodeName, "instanceDisplayType"),
                            annotation=annotation)
            ...
            self.addControl("instanceBoundingBox", label="Use bounding box",
                            changeCommand=lambda nodeName: self.updateDimming(nodeName, "instanceBoundingBox"),
                            annotation=annotation)
            ...
            self.addControl("inputLocalTranslationOffset", label="Local Translation Offset",
                            changeCommand=lambda nodeName: self.updateDimming(nodeName, "inputLocalTranslationOffset"),
                            annotation=annotation)

            annotation = "A translation offset in worldspace XYZ."
            self.addControl("inputGlobalTranslationOffset", label="Global Translation Offset",
                            changeCommand=lambda nodeName: self.updateDimming(nodeName, "inputGlobalTranslationOffset"),
                            annotation=annotation)

            self.addSeparator()

            annotation = "A rotation offset over the curve local space. This offset is initialized to the original object rotation. "
            self.addControl("inputLocalRotationOffset", label="Local Rotation Offset",
                            changeCommand=lambda nodeName: self.updateDimming(nodeName, "inputLocalRotationOffset"),
                            annotation=annotation)

            annotation = "A worldspace rotation offset."
            self.addControl("inputGlobalRotationOffset", label="Global Rotation Offset",
                            changeCommand=lambda nodeName: self.updateDimming(nodeName, "inputGlobalRotationOffset"),
                            annotation=annotation)

            self.addSeparator()

            annotation = "A scale offset over the object local space. This offset is initialized to the original object scale."
            self.addControl("inputLocalScaleOffset", label="Local Scale Offset",
                            changeCommand=lambda nodeName: self.updateDimming(nodeName, "inputLocalScaleOffset"),
                            annotation=annotation)

            self.endLayout()

            # Setting ramp contronls
            ...
            self.addControl("inputTransformMatrix", label="Input object",
                            changeCommand=lambda nodeName: self.updateDimming(nodeName, "inputTransformMatrix"),
                            annotation=annotation)
            ...
            self.addControl("inputShadingGroup", label="Shading Group",
                            changeCommand=lambda nodeName: self.updateDimming(nodeName, "inputShadingGroup"),
                            annotation=annotation)
    ...
    def buttonNew(self, nodeName):
        # documentation: pymel Buttons: https://help.autodesk.com/cloudhelp/2017/CHS/Maya-Tech-Docs/PyMel/generated/classes/pymel.core.uitypes/pymel.core.uitypes.Button.html?highlight=button#pymel.core.uitypes.Button
        ...
        self.resetAnglesButton.setAnnotation('When pressed, all the manipulators angles will be reset to 0.')
    ...
    def onResetManipPositions(self, nodeName):
        ...
        res = pm.confirmDialog(title='Confirm reset positions', message='Are you sure you want to reset the manipulators positionss?', button=['Yes', 'No'], defaultButton='Yes', cancelButton='No', dismissString='No')
        ...
        if res == 'Yes':
            ...
            if len(curve.connections()) == 1:
                ...
                # review: I think this method isn't accurate ↓
    ...
    def onResetManipAngles(self, nodeName):
        # First, show manips to update manip count
        self.onEditManipulators(nodeName)
        res = pm.confirmDialog(title='Confirm reset angles', message='are you sure you want to reset the manipularors angles?',  button=['Yes','No'], defaultButton='Yes', cancelButton='No', dismissString='No')
    ...
    def updateManipCountDimming(self, nodeName):
        # read attribute enableManipulators
        enableManips = pm.PyNode(nodeName).enableManipulators.get()

        # pymel object button class.
    ...
    def onInstanceModeChanged(self, nodeName):
        ...
        if pm.PyNode(nodeName).Type() == kPluginNodeName:
            ...
            if mode == 0:
                ...
                # documentation: dimControl: https://download.autodesk.com/us/maya/2010help/CommandsPython/editorTemplate.html#flagdimControl

# command
class instanceAlongCurveCommand(OpenMayaMPx.MPxCommand):
    def __init__(self):
        super(instanceAlongCurveCommand, self).__init__()
        self.mUndo = []
    ...
    def undoIt(self):
        ...
        for m in reversed(self.mUndo):
            m.undoIt()  # undo method commands, <- MDagModifier
    ...
    def findShadingGroup(self, dagPath):
        # Explanation: find shading group
        ...
        for child in xrange(dagPath.childCount()):
            ...
            fnDagNode.getPath(childDagPath)  # get dagPath and store in childDagPath
        ...
        if self.hasShapeBelow(dagPath):
            ...
            if instPlugArrayElem.isConnected():
                ...
                if connectedPlugs.length() == 1:
                    ...
                    # check if it is compatible with MFnSet
    ...
    def doIt(self, argList):
        try:
            ...
            if list.length() == 2:
                ...
                # Explanation: check if node is nurbsCurve type
...
class instanceAlongCurveLocatorManip(OpenMayaMPx.MPxManipContainer):
    # name of class is important -> '%sManip' % node
    # Documentation: http://around-the-corner.typepad.com/adn/2012/10/maya-custom-manipulators-.html
    def __init__(self):
    ...
    # you can combine diferent manipulatortypes
    def createChildren(self):
        ...
        self.manipIndexCallbacks = {}  # Review: study this

        selectedObjects = OpenMaya.MSelectionList()
        OpenMaya.MGlobal.getActiveSelectionList(selectedObjects)

        # TIP: Because we need to know the selected object to manipulate, we cannot manipulate various nodes at once...
        if selectedObjects.length() != 1:
            return None

        # check if manip options is activated
        dagPath = OpenMaya.MDagPath()
        ...
        # review: created Manip point and Manip disc, but where is the positions set?
    ...
    def getSortedCurveAxisArrayFromPlug(self, nodeFn, count):
        ...
        def getKey(item):
            return item[1]
        # Tip: sorted: reorder the values of a list
        # Documentation: sorted: https://developers.google.com/edu/python/sorting
    ...
    def connectToDependNode(self, node):
        ...
        try:
            ...
            expectedHandleCount = handleCountPlug.asInt()  # not in use
            ...
            if actualHandleCount > 0:
                # tip: math.ceil: the smallest integer not less than x. 3.14 -> 4
            ...
            for i in xrange(self.manipCount):
                ...
                # Handle data
                # parameter and angle attribute plugs
                ...
                # connecting manip
                fnCurvePoint = OpenMayaUI.MFnPointOnCurveManip(self.manipHandleList[i][0])  # first elemetn pointOnCurveManip
                fnCurvePoint.connectToCurvePlug(curvePlug)  # conencted to the curve attr
                fnCurvePoint.connectToParamPlug(curveParameterPlug)  # param attribute
                ...
                if i >= actualHandleCount:
                    ...
                    if actualHandleCount > 1:
                        ...
                        curveParameterPlug.setFloat(
                            self.curveFn.findParamFromLength(self.curveFn.length() * float(i) / float(self.manipCount)))
    ...
    def discAxisConversion(self, manipTuple):
        ...
        manipData = OpenMayaUI.MManipData(numDataObj)  # review: data type for manips?
        return manipData
...
def initializePlugin(mobject):
    ...
    try:
        # Explanation: .MGlobal.kBatch -> running no UI // MGlobal.kLibraryApp -> Running as a standalone (MLibrary) application.
        if (OpenMaya.MGlobal.mayaState() != OpenMaya.MGlobal.kBatch) and (OpenMaya.MGlobal.mayaState() != OpenMaya.MGlobal.kLibraryApp):
            ...
            mplugin.registerNode(kPluginManipNodeName, kPluginNodeManipId,
                                 instanceAlongCurveLocatorManip.nodeCreator,
                                 instanceAlongCurveLocatorManip.nodeInitializer,
        ...
        mplugin.registerNode(kPluginNodeName, kPluginNodeId, instanceAlongCurveLocator.nodeCreator,
                             instanceAlongCurveLocator.nodeInitializer, OpenMayaMPx.MPxNode.kLocatorNode,
                             kPluginNodeClassify)
...
def uninitializePlugin(mobject):
    ...
    try:
        ...
        if (OpenMaya.MGlobal.mayaState() != OpenMaya.MGlobal.kBatch) and (
                OpenMaya.MGlobal.mayaState() != OpenMaya.MGlobal.kLibraryApp):
...
def getSortedCurveAxisArray(mObject, curveAxisHandleArray, count):
    ...
    expectedHandleCount = OpenMaya.MFnDependencyNode(mObject).findPlug(
        instanceAlongCurveLocator.curveAxisHandleCountAttr).asInt()

    for i in xrange(min(expectedHandleCount, curveAxisHandleArray.elementCount())):
        curveAxisHandleArray.jumpToArrayElement(i)
        parameterHandle = curveAxisHandleArray.inputValue().child(
            instanceAlongCurveLocator.curveAxisHandleAttr.parameter)
